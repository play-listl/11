document.addEventListener('DOMContentLoaded', function() {
    ...
    shareButton.addEventListener('click', function() {
        ...
        const individualScores = userAnswers.map((answer, index) => {
            const userScore = scores[index].points;
            const maxScore = points[correctOrder[index].city][index];
            const isCorrect = (correctOrder[index].city === answer);
            const color = isCorrect ? 'green' : 'red';
            return `${index + 1}. <span style="color: ${color};">${userScore}/${maxScore}</span>`;
        }).join('<br>');

        const shareText = `${scoreMessage}<br><br>${individualScores}`;

        if (navigator.share) {
            navigator.share({
                title: 'Listl Score',
                text: shareText,
                html: shareText // Assuming `html` property is supported for HTML sharing
            }).then(() => {
    ...
    function showCorrectAnswers(userAnswers, scores) {
        ...
        userAnswers.forEach((answer, index) => {
            ...
            const userScore = scores[index].points;
            const maxScore = points[correctOrder[index].city][index];
            li.innerHTML = `${answer} (${correctOrder[index].city} - ${correctOrder[index].year}) <span style="color: ${answer === correctOrder[index].city ? 'green' : 'red'};">${userScore}/${maxScore}</span>`;
            answersList.appendChild(li);
        });
    }
